...
- **D30**: 시작 때 팁이 z_safe 아래면 거절하지 않고 같은 x · y · 현재 자세로 z_safe 까지 먼저 올린다.
- **D33 선 실패 뒤 계속**(`continue_on_line_failure`, 출발값 true): 한 선의 goal(접근 1 · 2 · 경로 · 후퇴)이 `ROBOT_ERROR(204)` 로 끝나면 그 선만 `FAILED` 로 적고, `/robot/sample` 로 팁 위치를 본다. z_safe 위면 바로 다음 선의 접근 1, 아래면 툴 축 뒤(−d)로 `approach_m` 물러난 뒤 같은 x · y 로 z_safe 까지 올린 뒤 다음 선. 팁 위치를 모르면 · 복구 이동이 실패하면 ERROR. **204 만이다** — 정지 · 취소 · 과대 외력 · 시간 초과 · 래치 · 604 는 그대로 ERROR · STOPPED. 끝까지 갔는데 FAILED 가 있으면 phase 는 DONE, 마무리 홈 복귀는 그대로 하고, `WeldResult.success=false` · `reason_code` = 첫 FAILED 선의 코드 · `detail` = `L1,L5 FAILED`. RunWeld `Result.success` 도 false. 같은 선을 다시 시도하지 않는다. 근거: 9/23 M1(#186)에서 L1 · L5 가 45° 자세로 도달 불가(D27).
