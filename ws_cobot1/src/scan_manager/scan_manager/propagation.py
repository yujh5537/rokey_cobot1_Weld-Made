...
원칙
- **감시 쪽을 먼저 바꾼다**(P03 → P02 → P01). safety_monitor 가 받아들이기만 하면, 뒤에서 무엇이
  실패해도 2차 감시는 요청한 값에 가 있고 어긋남은 "1차 감시가 옛 값에 남았다"로만 남는다.
  반대 순서면 2차가 옛 값에 남아, 관제자가 조인 줄 아는 임계가 감시에는 안 들어간 조합이 생긴다.
  (safety_monitor **자신이** 거절하면 어느 순서든 어긋난다. 그때는 아래의 쌍 검사가 START · RESUME
  을 막는다.)
- **되돌리지 않는다.** 되돌리기도 실패할 수 있고, 실패한 되돌리기는 더 알기 어려운 상태를 만든다.
  한 노드가 거절해도 남은 노드에 계속 보낸다 — 거기서 멈추면 어긋난 조합이 오히려 늘어난다.
...
# 한 노드에 여러 개를 보내도 rclpy 는 **파라미터마다 따로** 적용하고 결과도 따로 준다
# (SetParameters 는 원자적이지 않다). 그래서 거절은 그 이름 하나에만 걸리고, 같은 요청의 나머지는
# 들어간다. 상대 노드가 "A 는 B 보다 작아야 한다" 같은 교차 검증을 넣으면 아래 튜플의 순서가
# 결과를 바꾼다 — 지금 세 노드는 항목마다 독립으로만 검사한다(2026-09-21 확인).
