...
# ERROR 로 끝난 작업에서 /safety/reset 뒤 RESUME 을 허용하는 실패 사유 (계약 5.3 · 9장, v0.1.16).
#
    ...
    def _check(self, command, conditions, scan_id):
        ...
        if command is Command.RESUME:
            if phase is Phase.ERROR and not failure_is_resumable(self._failure):
                # 허용 목록 밖이다. 안전 점검 · 복귀 뒤 새 START 만 가능하다 (계약 9장, v0.1.16)
        ...
        *,
        ...
        with self._lock:
            ...
            if signal is Signal.FAILED:
                ...
                self._failure = failure
                # 재개 지점은 **허용 목록에 있는 사유일 때만** 남긴다 (계약 9장, v0.1.16).
