...
## 판정 규칙
- **`debounce_n` 이 세는 것 (v0.1.17 결정 9).** 연속 **샘플 수**이지 독립된 측정 횟수가 아니다.
  `get_tool_force` 값은 약 10 Hz(약 94 ms)로만 갱신되는데(`docs/env/api-check-log.md`) robot_manager 는
  더 빠르게(설정 50 Hz, 실기 실측 약 43 Hz) 조회해 발행하므로 **같은 값이 여러 샘플 연속으로 온다.**
  그래서 지금의 `debounce_n = 3` 은 사실상 "측정 1 회 + 약 44 ms 대기"다.
  - 그래도 **현행을 유지한다.** 실기에서 이 값으로 검증된 기록이 있고, 바꾸면 임계 · 편향 보정을 다시 재야 한다.
  - **"값이 바뀐 샘플 3 회"로 바꾸지 않는다.** 오프라인 계산(130 N/s)에서 판정 순간의 참 힘이
    중앙 34.8 N · 최대 40.6 N 이다 — 과대 외력 30 N 을 넘고 탐침이 밀리는 43 N 에 닿는다.
  - **robot_manager 의 스텝 모드 EDGE 는 별개다.** 멈춘 뒤 **새로 받은 샘플만** `step_force_samples` 개
    평균내고, 그 이벤트의 `debounce_count` 는 1 이다(계약 7.2). 두 규칙을 같은 것으로 읽지 않는다.
