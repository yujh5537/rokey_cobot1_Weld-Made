...
- **`/robot/status` 가 끊겨 있으면 `moving` 은 모르는 값이다.** 마지막으로 알려진 `moving=false` 를 믿으면, 상태가 끊긴 뒤
  움직이기 시작한 경우를 "서 있다"로 읽는다. 그래서 `ROBOT_STATUS_LOST` 가 참인 동안에는 최신성 조건을 **STOP 으로 본다**
  - 대가: 로봇을 세워둔 채 브링업만 내려도 STOP → 래치가 걸려 관제자가 안전 해제를 눌러야 한다. 통합 시험 중에 성가실 수 있다.
    그래도 "모르는 것을 안다고 하지 않는다"를 택했다(규칙 9). 브링업을 다시 띄우면 조건이 해소되어 `/safety/reset` 이 성공한다
  - 그때 `/robot/stop` 서버도 같이 없으므로 `detail` 에 "마지막 응답: /robot/stop 서버가 없다"가 남는다.
    드라이버가 무응답인 경우(노드는 떠 있다)와 구분해서 읽는다
- **`SAMPLE_STALE` 은 sim 종단 실행 중 robot_manager 가 죽으면 바로 뜬다.** 한 번도 받은 적 없으면 조용하고,
  받다가 끊기면 STOP 이다. 저녁 통합 로그에서 이것이 보이면 정상 동작이다
