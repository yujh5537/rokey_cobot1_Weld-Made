...
PARAMS = {
    'over_force_n': 30.0, 'drop_limit_m': 5 * MM, 'drop_limit_margin_m': 4 * MM,
...
def test_drop_limit_uses_first_slide_sample(rig):
    """2차 한계는 drop_limit_m(5 mm) + drop_limit_margin_m(4 mm) = 9 mm 다 (계약 7.2)."""
    rig.status(moving=True)
    rig.send(z=Z0, operation=RobotSample.OP_SLIDE)
    rig.send(z=Z0 - 8.9 * MM, operation=RobotSample.OP_SLIDE)
    assert rig.stop_requests == []
    rig.send(z=Z0 - 9.2 * MM, operation=RobotSample.OP_SLIDE)
...
def test_first_stage_window_does_not_latch_the_second(rig):
    ...
    rig.status(moving=True)
    rig.send(z=Z0, operation=RobotSample.OP_SLIDE)
    for drop_mm in (5.1, 6.0, 7.5, 8.9):
