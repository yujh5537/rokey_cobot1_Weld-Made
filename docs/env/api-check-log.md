...
| set_desired_force (DR_FC_MOD_REL) / release_force | service `force/set_desired_force`(`mod=1`), `force/release_force`. 래퍼 있음(기본 mod가 ABS라 REL을 명시해야 한다) | 호출 성공 | 미확인 | 2026-09-19 | 목표 힘 0 N, dir z. 1 s 동안 위치 변화 0.0 mm. 해제는 `finally`에서 불렀다. 0이 아닌 힘은 부르지 않았다(공중에서 로봇이 그 방향으로 움직인다). **mod 의미는 헤더에서 확인**(`dsr_msgs2/srv/detail/set_desired_force__struct.h`): ABS(0)=절대값, REL(1)=**호출 시점 상태 기준 상대값**. 실기에서 ① SLIDE 시작 직후 `|F − F0|` 1회, ② `DR_FC_MOD_ABS` 가 실제로 동작하는지 확인한다 (#73) |
