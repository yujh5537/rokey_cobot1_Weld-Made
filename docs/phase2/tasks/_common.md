...
## 작업 절차
1. **설계 → 자체 점검 → 코드.** high-level design(모듈 · 책임 · 자료 흐름 · 바꾸지 않는 것) → detail design(함수 · 클래스 시그니처, 자료 구조, 스레드 · 락, 오류 · 경계, 테스트 목록) → pseudo code(핵심 경로) 순서를 **권장**한다. 병후에게 브리핑하거나 확인받을 필요는 없다 — 세션이 스스로 점검하고 넘어간다(2026-09-24 병후 결정). 지시서의 초기 설계는 출발점이다 — 더 나은 안이 있으면 이유와 함께 바꿔 적용한다.
2. **임의 판단 금지.** 계약 · 코드 · 지시서가 서로 다르거나 불분명하면 추측하지 말고 담당자에게 묻는다(선택지 + 권고안). 질문은 모아서, 답과 무관한 부분은 계속 진행. **다른 사람 담당과 통신하거나 영향을 주는 변경(계약 · 인터페이스 · 공유 파라미터 · 공용 yaml)은 먼저 PR 로 확인받는다.** 그 밖은 유연하게.
