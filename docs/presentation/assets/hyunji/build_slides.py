...
def slide_kpi(prs, src):
    ...
    rows = [
        ...
        ['접촉 검출 하중', '5 N 이하', '임계 3 N: 평균 4.49 N, 10 회 중 2 회 초과(5.15 · 5.29)\n임계 2 N: 평균 2.76 N, 0/7 — '
         ...
         '임계 2 N 검토 — 하강 거짓 접촉 3/10(#182, settle 구간 6 N 판정 문제)과 같이'],
        ...
        ['관제 화면 반영 지연', '200 ms 이내', '최대 312 ms — 9/22(TR-05)', '미달', '최대값을 만든 토픽을 못 가렸다',
         '토픽별 지연 기록 재시험'],
        ['전체 탐색 시간', '120 s 이내', '438 s — 9/23 실기(scan …3702): 스텝 긁기 331 s(76 %) · 재접근 내림 3 회 76 s · '
         '나머지 35 s(#180)', '미달',
         '연속 밀기가 실기에서 실패 → 멈춰서 읽는 스텝 모드(#160). 120 s 는 연속 밀기 전제',
         '재접근 2 단 −60 · 거친 스텝 1.0 mm −150 → 약 230 s(계산값 · 미실시). 120 s 는 KPI 재설정(9/25)'],
        ['실기 연속 성공', '(목차 항목)', '웹 START → 형상 → 웹 표시 종단 성공 — 9/23(PR #179). 연속 회차 기록은 없다', '미확인', '—',
         '리허설에서 연속 회차 기록'],
        ['반복성', '10 회 σ 0.3 mm', '미실시 (같은 점 10 회 z 산포 0.040 mm 는 접촉 재현성 — 치수 반복성이 아니다)', '미실시',
         '—', 'TR-02'],
    ...
    notes(s, 'KPI 표입니다. 미달을 숨기지 않고 그대로 적었습니다.\n'
             ...
             '중지 반응은 sim 에서만 쟀고, 반영 지연은 최대 312 ms 로 미달입니다. 탐색 시간은 438 s 로 목표의 3.6 배인데, '
             '힘 제어로 연속해서 미는 방식이 실기에서 실패해 멈춰서 힘을 읽는 스텝 모드로 바꾼 대가입니다. 120 s 는 연속 밀기를 '
             '전제로 잡은 숫자였습니다. 실기 연속 성공 횟수는 기록이 없어 미확인으로 두었습니다.\n'
             '(출처: TR-01_20260923(PR #181) · TR-05_20260922 · #180 학민 정정(9/23) · 병후 정리(9/25) · PR #179. '
             '확인 필요: 연속 성공 회차 기록이 있으면 병후에게)')


def slide_kpi_story(prs, src):
    s = frame_slide(prs, src, '⑥ 미달을 어떻게 읽나 — 정밀도를 택해 시간 KPI 를 놓쳤다 → 원인 · 개선안 · KPI 재설정',
                    '* 04-⑥ 실기 결과 · KPI (2/2)')
    picture(s, 'kpi-scan-time.png', LEFT, 2.42, w=7.2)
    picture(s, 'kpi-detect-load.png', LEFT, 4.42, w=7.2)
    steps = [
        ('① 처음', 'KPI 를 "한 번 성공하면 된다"로 읽었다. 회차 기준 · 시간 기준을 늦게 챙겼다'),
        ('② 판단', '9/21~22 힘 제어 연속 밀기가 실기에서 실패(방향별 누름 1.5~8.6 N 편차 · 방향 전환 뒤 뜸 · 가짜 EDGE, '
                 '#152~#155) → 멈춰서 힘을 읽는 스텝 모드(#160)로 정확도 우선'),
        ('③ 결과', '치수 +1.50 / −0.36 / +0.26 mm 로 ±3 mm 달성. 대신 탐색 438 s(KPI 의 3.6 배) — 76 % 가 스텝 긁기. '
                 '120 s 는 연속 밀기를 전제로 잡은 숫자'),
        ('④ 정직하게', '검출 하중 평균 4.49 N 은 통과처럼 보이지만 회차 2/10 초과 → 미달로 적었다'),
        ('⑤ 개선안 · KPI', '재접근 2 단(−60 s) · 거친 스텝 0.5 → 1.0 mm(−150 s, 분해능은 다듬기 0.1 mm 가 정함) → 약 230 s, '
                          '계산값 · 미실시. 그 아래는 스텝당 비용의 한계 → 120 s 는 스텝 모드 기준으로 KPI 재설정(팀 결정 9/25)'),
    ]
    y = 2.42
    ...
    notes(s, '왜 미달이 났는지 순서대로 말씀드리겠습니다. 처음에는 KPI 를 한 번 성공하는 것으로 읽었고, 회차 기준과 시간 '
             '기준을 늦게 챙겼습니다. 9/21~22 실기에서 힘 제어로 연속해서 미는 방식이 실패했습니다 — 방향마다 실제 누름이 '
             '1.5 에서 8.6 N 까지 흩어지고, 방향을 바꾼 뒤 떠서 모서리를 놓치고, 가짜 EDGE 가 났습니다. 그래서 한 스텝 가고 '
             '멈춰서 힘을 읽는 스텝 모드로 바꾸며 정확도를 먼저 잡았습니다. 그 결과 치수는 목표 안에 들어왔지만 탐색 시간은 '
             '438 s 가 되었고, 위 그래프처럼 그중 76 % 가 스텝 긁기입니다. 120 s 는 연속 밀기를 전제로 잡은 숫자였습니다.\n'
             '아래 그래프는 검출 하중입니다. 빨간 막대 두 개가 5 N 을 넘은 회차이고, 이것 때문에 평균이 통과여도 미달로 '
             '적었습니다. 오른쪽은 임계 2 N 의 결과입니다.\n'
             '개선안은 계산값이고 아직 해 보지 않았습니다. 재접근 내림을 빠른 45 mm 와 저속 5 mm 로 나누면 세 번에 약 60 초, '
             '거친 스텝을 1 mm 로 올리면 약 150 초가 줄어 230 초 정도가 됩니다. 정확도는 다듬기 스텝 0.1 mm 가 정하므로 '
             '영향이 없습니다. 그 아래는 스텝마다 멈춰서 힘을 세 번 읽는 비용이라 방식의 한계이고, 그래서 120 초는 스텝 '
             '모드 기준으로 다시 잡아야 한다는 것을 숨기지 않고 말씀드립니다.\n'
             '(출처: #180 본문 · 학민 정정 9/23 · 병후 정리 9/25. 병후 정리의 200 s 는 재접근 4 회 기준이고 실측은 3 회라 230 s 로 적었다)')
