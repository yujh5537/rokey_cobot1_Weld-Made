...
flowchart LR
  ...
  SM -. "SetParameters" .-> SF
  RM --> DSR
  DSR -- "/dsr01/joint_states<br/>(JointState · 표시 전용)" --> BR
  ...
  SF -- "safety status" --> BR
```
scan_manager는 측정 · 판정에 `/robot/sample`을 쓰지 않는다 — 중단 위치는 `ExecuteMotion` Result의 정지 pose다. 다만 **마지막 유효 pose 하나는 구독해 들고 있다 [v0.1.21]**: 안전복귀(계약 7.5 ①)와 재시작(7.6)의 첫 모션은 "지금 어디 있는지"를 알아야 목표를 만들 수 있고, 기록된 좌표는 그때 로봇이 있는 자리가 아닐 수 있기 때문이다. 이름 · 필드 · QoS는 `docs/contracts/ros-interfaces.md`, MQTT는 `docs/contracts/mqtt-schema.md`.
...
| mqtt_bridge | ROS ↔ MQTT, 명령 ID 중복·만료 검사(중지는 만료 검사 제외)·접수/완료·연결 상태 추적, 단위 변환(m→mm), NaN→null, `/dsr01/joint_states` 표시 전용 중계(M0609/RG2 분리) | 의석 (T27은 병후와 공동) |
