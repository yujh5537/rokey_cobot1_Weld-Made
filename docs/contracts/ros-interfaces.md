...
상태: **v0.1 동결** (2026-09-18, T01 1·2차 회의) · v0.1.1(T09, QoS 정의 위치 확정 · 타입 변경 없음) · v0.1.4(2026-09-20, T15, PR #72 · #83: 6.3절 실측 발행 주기, 9장 `RobotStatus.moving`의 근거와 파라미터 변경 통지 · 타입 변경 없음) · v0.1.5(#69, 판정 샘플 = 첫 샘플 확정 · 타입 변경 없음) · v0.1.9(2026-09-21, 6.3절 발행 주기가 부하에 따라 달라짐 · 두 번째 실측과 공백 꼬리 추가 · 타입 변경 없음). 변경은 PR + `CHANGELOG.md`로만 한다.
...
| `/contact/tare` | `TareForce` | contact_detector | scan_manager | 외력 기준값 F₀ 설정(무접촉 · 정지) |
...
**실측 — `/robot/sample` 주기는 부하에 따라 크게 달라진다. 두 번 쟀고 두 배 가까이 벌어졌다 [v0.1.9].**
...
있었다(현지). `stale_age_ms`(scan_manager) · `sample_stale_ms`(safety_monitor) · EDGE 판정 창(contact_detector)의
근거를 평균 주기에 두면 이 공백에서 오판한다. 세 값은 **최대 공백**을 기준으로 잡고, 공백의 원인(드라이버 응답
지연인지 호스트 부하인지)은 미확인이다. 실기 주기와 공백 분포는 T24 전에 다시 잰다.
