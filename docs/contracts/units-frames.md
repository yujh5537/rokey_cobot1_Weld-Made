...
| 탐침 TCP 오프셋 (최하단점 기준) | **[-1.30, 3.71, 249.99] mm, 자세 [0, 0, 0] deg** (플랜지 기준, 불확도 약 ±1 mm). 아래 구 중심 z에 r을 더했다. 컨트롤러 이름 `rg2_probe_tip` | 2026-09-19 / 학민 (확인: 현지) · `docs/env/tool-tcp-register.md` 3절 |
| (실측) 탐침 팁 구 중심의 오프셋 (플랜지 기준, x · y · z, 자세) | [-1.30, 3.71, 249.76] mm, 자세 [0, 0, 0] deg (가정). 피벗 보정 14자세, 불확도 약 ±1 mm. 피벗 보정은 팁을 홈에 앉혀 했으므로 구한 점은 구 중심이다 | 2026-09-19 / 학민 (확인: 현지) · `docs/env/tool-tcp-register.md` 3절 |
| 툴 무게·무게중심 (RG2 + 탐침) | 1.3 kg, 무게중심 (0, 31.08, 29.84) mm (플랜지 기준). 펜던트에 있던 값이고 탐침 0.3 g은 무시했다. 무게중심 y 치우침의 출처는 미확인. 컨트롤러 이름 `rg2_probe` | 2026-09-19 / 학민 (확인: 현지) · `docs/env/tool-tcp-register.md` 2절 |
| 홈위치(시작위치) 관절각 | TBD | |
| 탐색 기준점(원점 상공) 높이 | TBD | |
| 팁 반지름 r | **0.225 mm** (인공눈물 용기 끝, 지름 0.45 mm 실측. 설계 출발값 3 mm를 대체) | 2026-09-19 / 학민 (확인: 현지) |
