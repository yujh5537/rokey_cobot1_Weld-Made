...
상태: **v0.1.2** (2026-09-19). 단위 · 기준 규칙은 확정이다. 좌표 **값**은 툴 무게 · TCP · 팁 반지름만 T02에서 실측했고, 나머지는 미확정이다. BRD 6장에 따라 물품 도착·환경 세팅 후 학민이 확정한다(T02, T03). TBD 값은 코드에 상수로 넣지 않는다. 실측값도 코드에 박지 않고 `contact_scan_bringup/config/real.yaml` 파라미터로 둔다.
