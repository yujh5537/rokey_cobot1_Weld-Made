...
- **z 계열 값은 전부 잠정이다**: z=0 100.6 · 홈 팁 높이 190.84 · 홈→윗면 111.92 · `max_descend_m` 0.117 · 0.1908. TCP z 248.52 에 종속된다. "탐침이 되돌아왔다"는 확인되지 않았고(`table_check_after_fix` 가 그 전과 0.004 mm 차이), 9/19 와 9/20 은 작업대의 **다른 자리**를 재서 두 평균을 뺀 계산이 평탄을 가정한다. 다음 세션에 9/19 의 두 점을 같은 자리에서 다시 찍어 확정한다
