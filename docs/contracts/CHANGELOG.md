...
## v0.1.3 (2026-09-19, T03, PR #TBD)
타입 변경 없음. `units-frames.md`의 **z=0**, **작업대 원점(초안)**, **축 평행**, **홈 관절각**을 실측해 적었다. 측정은 모두 플랜지 posx(`get_current_tool_flange_posx`)를 읽고, TCP [-1.30, 3.71, 249.99]를 적용해 팁 위치로 바꿨다. TCP 적용 상태에 영향을 받지 않도록 이렇게 했다. 영향: scan_manager(`base_to_fixture`, 결과 변환) · robot_manager(홈) · safety_monitor(작업영역) · sim 가상 직육면체.
- z=0 = 작업대 표면, Base z 100.6 mm. 기준 큐브(80 mm) 윗면은 180.7 mm로, 차이 80.1 mm가 캘리퍼 값과 맞는다
- 작업대 원점(초안) = 큐브 윗면 가운데 x · y + z=0 = (423.5, -187.1, 100.6) mm. 배치 가이드가 없어 정의는 리뷰에서 확정한다
- 축 평행: 가이드 대신 큐브 네 모서리로 확인했고, Base 축과 0.1~1.5° 차이였다
- 홈 관절각 [-24.14, 17.03, 51.68, -0.18, 111.39, -204.84] deg
- 탐색 기준점(원점 상공) 높이는 여전히 TBD다
