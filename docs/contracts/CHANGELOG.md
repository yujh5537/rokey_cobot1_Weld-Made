...
## v0.1.18 (2026-09-23, #147 새 홈 · 탐침 미교체)
**타입 변경 없음.** `units-frames.md` 홈 관절각을 **[-21.19, 15.24, 52.97, -0.08, 111.80, -15.14] deg** 로 바꿨다(2026-09-22 작업대 교체 뒤 학민이 다시 잡음, J6 을 ±180° 안으로). 옛 홈과 그 팁 좌표 · 높이는 무효다. 9/21 이후 탐침을 바꾸지 않았음을 확인해 TCP [0, 0, 252.12] 와 팁 반지름 0.225 mm 를 그대로 둔다(#147 의 교체 계획은 실행되지 않음). real 의 `detect_latency_s` 를 0.020 → 0.0 으로(스텝 모드는 멈춘 뒤 EDGE 확정이라 속도 × 지연 항이 없다, v0.1.15 7.2 의 후속). 영향: robot_manager(`home_joint_deg`) · scan_manager(편향 보정, 실기 값만). **작업대 원점 · z=0 · `search_origin_pose` · `max_descend_m` 은 재측정이 끝나지 않아 이 버전에 넣지 않았다**(#142). 번호: #161 이 v0.1.17 을 쓴다.
