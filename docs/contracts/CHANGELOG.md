...
- 탐색 기준점 높이(초안): 첫 하강은 홈 팁 높이(작업대 위 189.4 mm)에서 시작하고 별도 파라미터를 두지 않는다. 방향 전환은 `ros-interfaces.md` 7.3을 따른다. `max_descend_m` 하한 0.115 m(80 mm 부재, 여유 5 mm 포함), 상한 < 0.189 m − 안전 여유
