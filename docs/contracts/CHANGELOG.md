...
- 2026-09-25(병후 결정): **D12 갱신** — P1 robot_manager `ExecutePath` 구현은 현지(PR #191), 리뷰 학민. **D32** — `path_tolerance_m` 은 line 모드의 중간 점 도착 판정에도 쓰고 `≤ 0` · NaN 은 604(현지 해석 채택). **D31 결과** — spline 은 Virtual 응답 지연으로 사용 불가(현지 9/24), line 만. 5.2 에 명령 실패 뒤 정지 확인 규칙(#191 · #193). `weld-motion.md` 5절에 선 사이 이동은 z_safe 두 점 사이에서만(학민 M1 충돌 사례). 타입 변경 없음
