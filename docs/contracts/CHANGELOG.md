...
- 탐색 기준점 높이(초안): ① 첫 하강은 홈 팁 높이(작업대 위 189.4 mm)에서 ② z_top 확정 후는 윗면 위 30 mm. `max_descend_m` 하한 ① 0.110 m ② 0.030 m
- 리뷰 반영(PR #60): 원점 y를 -187.1(T02 피벗 고정점)에서 네 모서리 가운데 -186.06으로 정정해 정의와 값을 맞췄다. 축 평행을 판정 불가로 바꿨다. 윗면 10점 산포(179.3~181.5)를 적었다. 기준 큐브 모서리 라운드 가설(폭 약 3.4 mm 과소 추정 가능)을 추가했다. 측정 원본을 PR 본문에서 `docs/env/origin-home-register.md`로 옮겼다
