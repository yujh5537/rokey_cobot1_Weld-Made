...
## v0.1.15 (2026-09-22, 환경 전면 재설치 · 탐침 교체로 좌표 값 무효화, #147)
타입 변경 없음. **값을 새로 정하지 않고, 쓰면 안 되는 값을 표시만 한다.** `units-frames.md` 의 좌표 값 표 8 개 행과 배치 원칙 1 · 2 · 4, 탐침 상태 전제조건에 무효 표시를 붙였다. 영향: scan_manager(`search_origin_pose` · `base_to_fixture` · `max_descend_m` · `tip_radius_m`) · contact_detector(접촉 임계의 강성 근거) · robot_manager(TCP 등록) · 실기 세션 절차 전부.
- **왜**: 기존 환경에 기울어짐이 있어 2026-09-22 에 작업대 · 받침대 · 큐브를 전부 뜯고 다시 설치했다. 탐침도 같이 교체한다. 옛 값을 실기에 그대로 넣으면 팁이 엉뚱한 자리로 내려간다
- **재설치로 무효**: 작업대 원점 (423.56, −186.06, 100.503) · z=0 100.503 · `search_origin_pose` · `max_descend_m` 하한 0.113 / 상한 0.1823 · 축 평행 2.1° · 지지면 평탄 3 점 · 테이프선 · "홈 → 윗면 107.44 mm" · 배치 원칙 1 · 2 · 4
- **탐침 교체로 무효**: TCP [0, 0, 252.12] · 팁 반지름 0.225 mm · 툴 무게 · 무게중심 · 홈 팁 높이 187.34 mm · 접촉 강성 43 N/mm · 무접촉 |F0| 1.81 N
- **유지**: 홈 관절각(로봇 본체는 건드리지 않는다) · 단위 · 프레임 규칙 · 큐브 규격 80 mm · 배치 원칙 3 · 5 · 6 · 힘 계열 파라미터(임계는 강성이 바뀌므로 T24 에서 다시 본다)
- **순서 제약**: 탐침 교체와 재설치가 겹쳐 TCP z 와 z=0 을 동시에 잃었다. 기존 교체 절차(알고 있는 z=0 을 접촉해 차이로 TCP z 를 구함, v0.1.11)를 쓸 수 없다. 자유도가 하나뿐이므로 ① 캘리퍼로 TCP z 잠정 등록 → ② 지지면 접촉으로 z=0 확정 → ③ 큐브 윗면 높이를 캘리퍼 80 mm 와 대조해 검산 순서로 닫는다
- **다음**: 재측정 뒤 #147 에서 작업대 원점 정의(자유 배치이므로 기준이 홈에서 `search_origin_pose` 로 옮겨간다) · 배치 원칙 1 · 2 · 4 · 새 좌표 값을 고쳐 쓴다. #141(하강점과 원점의 2.96 mm 어긋남)의 결정 4 개도 그때 함께 답한다. 그 전까지 #142(`real.yaml` 좌표 켜기)와 T24 · T25 · T30 · T34 는 진행할 수 없다
- TR-02 · TR-03 은 이번 MVP 에서 배제한다(병후 결정). 계약 문서에 걸린 값은 없다
