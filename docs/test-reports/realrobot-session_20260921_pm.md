...
sod && cd ~/ws_cobot_pjt
python3 docs/env/apply_tool_tcp.py          # 마지막 줄 OK: tcp=rg2_probe_tip [-1.3, 3.71, 252.12] (#138 머지 뒤: [0.0, 0.0, 252.12])
...
| | x | y | z | 회전 · 잔차 |
|---|---|---|---|---|
| ~~17:44 xy_0 · xy_180~~ | 422.62 | −187.03 | 287.81 | **무효**: J6 를 돌리지 않았다(회전 −0.0°) |
| 19 시 ① (펜던트, J6 −200.90) | 305.03 | −110.53 | 355.45 | 표시 TCP `Dripper_DA_1` · 다른 좌표계라 값 자체는 쓰지 않는다 |
| 19 시 ② (J6 −20.90) | 305.06 | −110.55 | 355.44 | 회전 −178.9°, 잔차 0.01 mm. **팁이 십자 위 그대로** |
| 재확인: 툴 Rz ±90° (TCP [0, 0, 252.12], PC `move_line` ref=TOOL) | | | | **팁 제자리** |
| 새 TCP 홈 팁 | 424.40 | −183.22 | 287.61 | 홈 플랜지 x · y (424.46, −183.09)와 0.14 mm |

- 결과 TCP x · y: **(0.00, 0.00)** — 옛 값 (−1.30, 3.71)이면 180° 에 7.8 mm 옮겨 간다. 계약 반영 #137 / PR #138
- 펜던트 조그 중에는 PC 연결이 끊긴다. 펜던트에서 찍고 표시 TCP 를 적거나, PC 에서 `move_line`/`move_joint` 상대 이동으로 한다
- **이후 명령의 절대 x · y**: 기준점 (525.19, −172.09) 은 실제 위치라 새 TCP 에서도 그대로 쓴다. 옛 TCP 로 기록한 오늘 좌표(4번 x 464.49 등)는 실제보다 (−1.25, −3.73) mm 어긋나 있다
