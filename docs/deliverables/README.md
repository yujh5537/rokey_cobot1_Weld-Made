...
| 제출물 4 종 (파일명의 조 번호 · 표기는 병후 확인) | 영상(.mp4) · 발표자료(.pdf) · 소스코드(.zip, src 만 · build/install/log 제외) · Readme.md(시스템 설계 · 플로우차트, OS 환경, 장비 목록, 의존성 requirements.txt, 실행 순서 launch · 스크립트). 파일명 `A-1_협동1_조원1_…_조원6(중도포기).확장자` (중도포기자 반드시 기재) |
